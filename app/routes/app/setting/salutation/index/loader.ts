...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    const filterParts = ["ms.deleted_at is null"];

    const queryParams: unknown[] = [];

    if (search) {
      filterParts.push(
        `(ms.name ILIKE $${queryParams.length + 1} or ms.code ILIKE $${queryParams.length + 1} or ms.description ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_salutation ms WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((salutation) => ({
      ...salutation,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_salutation ms WHERE ${filter}`;
