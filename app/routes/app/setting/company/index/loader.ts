...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    if (search) {
      filterParts.push(
        `(mc.name ILIKE $${queryParams.length + 1} or mc.code ILIKE $${queryParams.length + 1} or mc.display_name ILIKE $${queryParams.length + 1} or mc.email ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_company mc WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((company) => ({
      ...company,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_company mc WHERE ${filter}`;
