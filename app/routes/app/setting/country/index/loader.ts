...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    if (search) {
      filterParts.push(
        `(mc.name ILIKE $${queryParams.length + 1} or mc.code ILIKE $${queryParams.length + 1} or mc.iso_code ILIKE $${queryParams.length + 1} or mc.iso_name ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_country mc WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((country) => ({
      ...country,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_country mc WHERE ${filter}`;
