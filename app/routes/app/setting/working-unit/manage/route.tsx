...
import { workingUnitFormLabel, schema } from "./constant";
...
const WorkingUnitForm = () => {
  const lastResult = useActionData<typeof action>();
  const data = useLoaderData<typeof loader>();

  const workingUnit = data.data;
  ...
  const [form, fields] = useForm({
    ...
    defaultValue: {
      id: workingUnit?.id,
      code: workingUnit?.code,
      name: workingUnit?.name,
      description: workingUnit?.description,
      active: workingUnit ? (workingUnit.active ? "y" : "n") : "y",
    },
  });
  ...
  return (
    <>
      <Form
        method="POST"
        {...getFormProps(form)}
        action="/app/setting/working-unit/manage"
      >
        <AppCardForm
          isForm={true}
          title="Form Master Unit Kerja"
          actionButtons={
            <div className="inline-flex gap-3">
              <Button
                ...
                variant="default"
                onClick={() => navigate("/app/setting/working-unit")}
        ...
        >
          <div className="grid grid-cols-2">
            <div className="p-5">
              ...
              <InputText
                fields={fields}
                {...workingUnitFormLabel["code"]}
                my="sm"
              />
              <InputText
                fields={fields}
                {...workingUnitFormLabel["name"]}
                my="sm"
              />
            </div>
            <div className="p-5">
              <Textarea
                {...workingUnitFormLabel["description"]}
...
export default WorkingUnitForm;
