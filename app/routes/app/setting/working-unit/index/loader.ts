...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    const filterParts = ["mwu.deleted_at is null"];

    const queryParams: unknown[] = [];

    if (search) {
      filterParts.push(
        `(mwu.name ILIKE $${queryParams.length + 1} or mwu.code ILIKE $${queryParams.length + 1} or mwu.description ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_working_unit mwu WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((workingUnit) => ({
      ...workingUnit,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_working_unit mwu WHERE ${filter}`;
