...
export const actionHandler = async (request: ActionFunctionArgs["request"]) => {
  try {
    ...
    const unit = await prisma.workingUnit.findFirst({
      where: { id: Number(id) },
    });
    ...
    if (action === "delete") {
      ...
      return redirect("/app/setting/working-unit");
    }

    const session = await formSessionStorage.getSession();
    session.set("unit_id", id);

    return redirect("/app/setting/working-unit/manage", {
      headers: {
