...
import { identityFormLabel, schema } from "./constant";
...
const IdentityForm = () => {
  ...
  const [form, fields] = useForm({
    ...
    defaultValue: {
      ...
      code: identity?.code,
      name: identity?.name,
      min_length: identity?.min_length?.toString(),
      max_length: identity?.max_length?.toString(),
      active: identity ? (identity.active ? "y" : "n") : "y",
    },
  });
  ...
  return (
    <>
      <Form
        method="POST"
        {...getFormProps(form)}
        action="/app/setting/identity/manage"
      >
        <AppCardForm
          isForm={true}
          ...
          actionButtons={
            <div className="inline-flex gap-3">
              <Button
                ...
                variant="default"
                onClick={() => navigate("/app/setting/identity")}
        ...
        >
          <div className="grid grid-cols-2">
            <div className="p-5">
              ...
              <InputText
                fields={fields}
                {...identityFormLabel["code"]}
                my="sm"
              />
              <InputText
                fields={fields}
                {...identityFormLabel["name"]}
                my="sm"
              />
            </div>
            <div className="p-5">
              <InputNumber
                fields={fields}
                {...identityFormLabel["min_length"]}
                my="sm"
              />
              <InputNumber
                fields={fields}
                {...identityFormLabel["max_length"]}
                my="sm"
              />
              <InputRadio
