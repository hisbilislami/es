...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    const filterParts = ["mi.deleted_at is null"];

    const queryParams: unknown[] = [];

    if (search) {
      filterParts.push(
        `(mi.name ILIKE $${queryParams.length + 1} or mi.code ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_identity mi WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((identity) => ({
      ...identity,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_identity mi WHERE ${filter}`;
