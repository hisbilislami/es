...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    const filterParts = ["mt.deleted_at is null"];

    const queryParams: unknown[] = [];

    if (search) {
      filterParts.push(
        `(mt.name ILIKE $${queryParams.length + 1} or mt.code ILIKE $${queryParams.length + 1} or mt.offset_sdt ILIKE $${queryParams.length + 1} or mt.offset_dst ILIKE $${queryParams.length + 1} or mt.abbrevation_sdt ILIKE $${queryParams.length + 1}  or mt.abbrevation_dst ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_timezone mt WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((timezone) => ({
      ...timezone,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_timezone mt WHERE ${filter}`;
