...
export const loaderHandler = async (request: LoaderFunctionArgs["request"]) => {
  try {
    ...
    const filterParts = ["jp.deleted_at is null"];

    const queryParams: unknown[] = [];

    if (search) {
      filterParts.push(
        `(jp.name ILIKE $${queryParams.length + 1} or jp.code ILIKE $${queryParams.length + 1} or jp.description ILIKE $${queryParams.length + 1})`,
      );
    ...
    const query = `SELECT * from esign.m_job_position jp WHERE ${filter} LIMIT $${queryParams.length + 1} OFFSET $${queryParams.length + 2}`;
    ...
    const serializedQueryResult = queryResult.map((jobPosition) => ({
      ...jobPosition,
    }));

    const queryTotal = `SELECT COUNT(*) as total FROM esign.m_job_position jp WHERE ${filter}`;
